...
final class MyViewControllerTests: XCTest {
    ...
    func testItResetsTheViewBackgroundColourWhenTheAlertIsDismissed() {
        givenAViewController()
        whenTheViewLoads()
        whenTheUserPerformsAnActionThatTriggersAnError()
        let okAction = parentViewController.receivedViewControllerForPresentation?.actions.first
        okAction?.invokeHandler()
        XCTAssertEqual(myViewController.view.backgroundColor, .systemBackground)
    }
